...
TEST_CSV = config.PROJECT_TEST_CSV # Use project test set for evaluation
...
try:
    ...
    y_test_true = test_df['price'].values # True prices for evaluation
...
del test_deep_features, test_ipq # Free memory
gc.collect()

# 2. Load K-Fold Models and Average Predictions
...
final_predictions = np.clip(final_predictions, 0, None) # Ensure non-negative prices

print("Averaged predictions from K-Fold models.")

# 3. Calculate Final SMAPE Score on the Project Test Set
