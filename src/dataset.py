...
class MultimodalDataset(Dataset):
    def __init__(self, csv_path, tokenizer, image_transform, image_dir=config.IMAGE_DIR, is_test_set=False):
        try:
            ...
            self.df = pd.DataFrame() # Return empty dataset
        self.tokenizer = tokenizer
        self.image_transform = image_transform
        self.image_dir = image_dir
        self.max_length = config.MAX_TEXT_LENGTH
        self.is_test_set = is_test_set # Flag to know if prices exist
    ...
    def __getitem__(self, idx):
        ...
        item = {
            ...
            'sample_id': str(row['sample_id']) # Ensure ID is string
        }

        # Include target only if it's not the final test set and price exists
        ...
        elif not self.is_test_set: # Handle case where price might be missing in train data
             item['target'] = torch.tensor(0.0, dtype=torch.float32) # Placeholder, consider logging warning
